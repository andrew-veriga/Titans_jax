...
self.memory_gate_proj = flax_nn.Dense(
    ...
    bias_init=flax_nn.initializers.constant(0.0),  # sigmoid(0) = 0.5
...
2. **`bias_init=0.0`:** `sigmoid(0) = 0.5` — сбалансированный старт. Memory вносит 50%, local_attn (Gemma weights) — 50%. Низкий gate масштабирует градиенты к memory params, замедляя их обучение.
